...
//
CCard* CCardList::operator[](const int nIndex) const { VERIFY(nIndex < m_numCards); return m_cards[nIndex]; }

CCard* CCardList::GetAt(const int nIndex) const { VERIFY(nIndex < m_numCards); return m_cards[nIndex]; }
...
int CCardList::GetTopCardVal() const { VERIFY(m_numCards > 0); VERIFY(m_bSorted); return m_cards[0]->GetFaceValue(); }

int CCardList::GetBottomCardVal() const { VERIFY(m_numCards > 0); VERIFY(m_bSorted); return m_cards[m_numCards - 1]->GetFaceValue(); }

int CCardList::GetNumCardsAbove(const CCard* pCard) const { return GetNumCardsAbove(pCard->GetFaceValue()); }

int CCardList::GetNumCardsBelow(const CCard* pCard) const { return GetNumCardsBelow(pCard->GetFaceValue()); }
