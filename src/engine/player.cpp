...
//
void CPlayer::RestoreInitialHand() { m_pHand->RestoreInitialHand(); }

void CPlayer::AddCardToHand(CCard* pCard, BOOL bSort) { m_pHand->Add(pCard, bSort); }

void CPlayer::AddCardToInitialHand(CCard* pCard, BOOL bSort) { m_pHand->AddToInitialHand(pCard, bSort); }
...
void CPlayer::RemoveAllCardsFromHand() { m_pHand->ClearHand(FALSE); }

void CPlayer::SetCard(int nIndex, CCard* pCard) { m_pHand->SetCard(nIndex, pCard); }

void CPlayer::SetInitialHandCard(int nIndex, CCard* pCard) { m_pHand->SetInitialHandCard(nIndex, pCard); }

void CPlayer::SortHand() { m_pHand->Sort(); }

void CPlayer::InitialEvaluateHand() { m_pHand->EvaluateHoldings(); }

void CPlayer::CountCards() { m_pHand->CountCards(); }

double CPlayer::CountCardPoints(BOOL bForceCount) { return m_pHand->CountPoints(bForceCount); }

int	CPlayer::GetNumCards() const { return m_pHand->GetNumCards(); }

int CPlayer::GetNumCardsInSuit(int nSuit) const { return m_pHand->GetSuitLength(nSuit); }

int CPlayer::GetSuitLength(int nSuit) const { return GetNumCardsInSuit(nSuit); }

BOOL CPlayer::HasCard(const CCard* pCard) const { return m_pHand->HasCard(pCard); }

BOOL CPlayer::HasCard(int nDeckValue) const { return m_pHand->HasCard(nDeckValue); }

CCard* CPlayer::GetCardByValue(int nVal) const { return m_pHand->GetCardByValue(nVal); }

CCard* CPlayer::GetCardByPosition(int nIndex) const { return m_pHand->GetAt(nIndex); }

CCard* CPlayer::GetCardInSuit(int nSuit, int nIndex) const { return m_pHand->GetCardInSuit(nSuit, nIndex); }

CCard* CPlayer::GetDisplayedCard(int nIndex) const { return m_pHand->GetDisplayedCard(nIndex); }

CCard* CPlayer::GetInitialHandCard(int nIndex) const { return m_pHand->GetInitialHandCard(nIndex); }

BOOL CPlayer::AreCardsExposed() const { return m_pHand->AreCardsExposed(); }
...
BOOL CPlayer::SuitHasCard(int nSuit, int nCardFaceValue) const { return m_pHand->SuitHasCard(nSuit, nCardFaceValue); }

double CPlayer::GetHCPoints() const { return m_pHand->GetHCPoints(); }

double CPlayer::GetTotalPoints() const { return m_pHand->GetTotalPoints(); }

int CPlayer::GetBalanceValue() const { return m_pHand->GetBalanceValue(); }

double CPlayer::GetNumQuickTricks() const { return m_pHand->GetNumQuickTricks(); }

int CPlayer::GetNumSuitsStopped() const { return m_pHand->GetNumSuitsStopped(); }
...
BOOL CPlayer::IsGameBid(int nBid) { return m_pBidder->IsGameBid(nBid); };

int CPlayer::GetGameBid(int nSuit) { return m_pBidder->GetGameBid(nSuit); };

void CPlayer::RecordBid(int nPos, int nBid) { m_pBidder->RecordBid(nPos, nBid); }

int CPlayer::InquireLastBid() { return m_pBidder->m_nBid; }

int CPlayer::GetNumBidsMade() { return m_pBidder->GetNumBidsMade(); }

int CPlayer::GetNumBidTurns() { return m_pBidder->GetNumBidTurns(); }

BOOL CPlayer::OpenedBiddingForTeam() { return m_pBidder->OpenedBiddingForTeam(); }

int CPlayer::GetPriorSuit(int nIndex) { return BID_SUIT(GetPriorBid(nIndex)); }

void CPlayer::ClearConventionStatus(CConvention* pConvention) { m_pBidder->ClearConventionStatus(pConvention); }
...
CCard* CPlayer::PlayForDummy() { return m_pDeclarer->PlayForDummy(); }

void CPlayer::RecordCardPlay(int nPos, CCard* pCard) { m_pPlayEngine->RecordCardPlay(nPos, pCard); }

void CPlayer::RecordCardUndo(int nPos, CCard* pCard) { m_pPlayEngine->RecordCardUndo(nPos, pCard); }

void CPlayer::RecordTrickUndo() { m_pPlayEngine->RecordTrickUndo(); }

void CPlayer::RecordRoundComplete(int nPos, CCard* pCard) { m_pPlayEngine->RecordRoundComplete(nPos, pCard); }

void CPlayer::RecordHandComplete(int nResult) { m_pPlayEngine->RecordHandComplete(nResult); }

void CPlayer::RecordSpecialEvent(int nCode, int nParam1, int nParam2, int nParam3) { m_pPlayEngine->RecordSpecialEvent(nCode, nParam1, nParam2, nParam3); }

int CPlayer::GetNumClaimableTricks() { return m_pPlayEngine->GetNumClaimableTricks(); }


//
// feeback routines (using m_pStatusDlg)
//
void CPlayer::SuspendTrace() { m_pStatusDlg->EnableTrace(FALSE); }

void CPlayer::ResumeTrace() { m_pStatusDlg->EnableTrace(TRUE); }

void CPlayer::Trace(const CString& strText) { *m_pStatusDlg << strText & "\n"; }

void CPlayer::AddTrace(CString& strMessage, CString& strTemp) { *m_pStatusDlg << strMessage << strTemp; }

void CPlayer::AddTrace(CString& strMessage, LPCTSTR szTemp) { *m_pStatusDlg << strMessage << szTemp; }

void CPlayer::ClearAnalysis() { m_pStatusDlg->Clear(); }

void CPlayer::ShowAnalysis(BOOL bShow) { m_pStatusDlg->Show(bShow); }

void CPlayer::SetTestBiddingMode(bool bMode) { m_pBidder->SetTestBiddingMode(bMode); }

bool CPlayer::GetTestBiddingMode() { return m_pBidder->GetTestBiddingMode(); }

Position CPlayer::GetOpeningPosition() { return (Position)m_pBidder->GetOpeningPosition(); }
