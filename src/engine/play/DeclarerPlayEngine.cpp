...
#include "engine/play/Cash.h"
#include "engine/play/TrumpPull.h"
#include "engine/play/Force.h"
#include "engine/play/Type1Finesse.h"
#include "engine/play/Type2Finesse.h"
#include "engine/play/Type3Finesse.h"
...
#include "engine/play/ExitPlay.h"
#include "gib.h"
...
void CDeclarerPlayEngine::RestartPlay()
{
  ...
  app_->UpdateStatusWindowWithPlayPlanAndSuitStatus();
}
...
{ 
	...
	if ( ((app_->GetDeal()->GetDeclarerPosition() == NORTH) || (app_->GetDeal()->GetDeclarerPosition() == SOUTH)) &&
		  (app_->GetDeal()->GetNumTricksPlayed() == 0) && (app_->GetDeal()->GetNumCardsPlayedInRound() == 2))
	{
		InitCombinedHand();
	}
  ...
  app_->UpdateStatusWindowWithSuitStatus();
}
...
{ 
	...
	{
		// call GIB
		CGIB& gib = app_->GetGIB();
		status << "3GIB! Invoking GIB for the play...\n";
		int nPlay = app_->InvokeGIB(gib, m_pPlayer, m_pHand, m_pPartnersHand, m_pStatusDlg);
  ...
  app_->UpdateStatusWindowWithPlayPlan();
	//
	return pCard;
}
...
CCard* CDeclarerPlayEngine::PlayForDummy()
{
	...
	if ((app_->GetDeal()->GetPlayRound() > 0) && (app_->GetDeal()->GetNumCardsPlayedInRound() < 2))
		ReviewPlayPlan();
...
void CDeclarerPlayEngine::TestForClaim()
{
	...
	if (app_->GetDeal()->GetNumCardsPlayedInRound() == 0)
	{
		...
		int numRemainingTricks = 13 - app_->GetDeal()->GetNumTricksPlayed();
		...
		// check entries
		EvaluateEntries();
		BOOL bPlayingInHand = (app_->GetDeal()->GetCurrentPlayer() == m_pPlayer);
		...
		BOOL bHandsWereExposed = m_pPlayer->AreCardsExposed() || app_->AreCardsFaceUp();
		m_pPlayer->ExposeCards(TRUE, !bHandsWereExposed);
		int nCode = AfxMessageBox(app_->FormString("Declarer (%s) claims the remaining tricks.  Do you agree?", app_->PositionToString(m_pPlayer->GetPosition())), MB_ICONQUESTION | MB_YESNO);
		m_pPlayer->ExposeCards(bHandsWereExposed, !bHandsWereExposed);
		if (nCode == IDYES)
		{
			// player concedes
			app_->GetDeal()->ClaimTricks(m_pPlayer->GetPosition());
			throw 1;	// escape
		}
		else
		{
			// player rejects the claim -- continue
      app_->RefreshScreen();	// clear the dialog
...
CCard* CDeclarerPlayEngine::TrySelectedPlays()
{
	...
	int nCurrPos = app_->GetDeal()->GetCurrentPlayerPosition();
...
//			m_playPlan.DeferPlay(pPlay);
	...
	if (pCard)
	{
		...
		if (app_->GetDeal()->GetCurrentPlayerPosition() == m_pPlayer->GetPosition())
...
CCard* CDeclarerPlayEngine::PlayThird()
{
	...
	CCard* pDeclarersCard = app_->GetDeal()->GetCurrentTrickCard(GetPlayerPosition());
	CCard* pDummysCard = app_->GetDeal()->GetCurrentTrickCard(GetPartnerPosition());
...
//	status << "PLYE3! Playing best card.\n";
	...
	CCard* pDeclarerCard = app_->GetDeal()->GetCurrentTrickCard(GetPlayerPosition());
	CCard* pDummysCard = app_->GetDeal()->GetCurrentTrickCard(GetPartnerPosition());
...
void CDeclarerPlayEngine::PlanPlay()
{
	...
	status << "2PLN10! We have" & 
			  ...
			  " for the " & app_->ContractToString(m_nContract, m_nContractModifier) & " contract.\n";
...
void CDeclarerPlayEngine::ReviewPlayPlan()
{
	...
	// starting as declarer
	status << "2PLNADJ! Reviewing play plan for round " & (app_->GetDeal()->GetNumTricksPlayed()+1) & "...\n";
	m_declarerPriorityDiscards.Clear();
	m_dummyPriorityDiscards.Clear();
	m_declarerDiscardedPriorityDiscards.Clear();
	m_dummyDiscardedPriorityDiscards.Clear();
...
int CDeclarerPlayEngine::CreateNoTrumpPlayPlan()
{
	...
	CPlayerStatusDialog& status = *m_pStatusDlg;
	// set verbosity level
	if (app_->GetDeal()->GetNumTricksPlayed() == 0)
		status.SetStreamOutputLevel(4);
	else
...
{	
	...
	if (nSuitsList.GetSize() == 0)
	{
		...
		int nIndex = 0;
		if (m_pCombinedHand->GetSuitsByLength(nIndex) == app_->GetDeal()->GetTrumpSuit())
			nIndex++;
...
//			CFinesse* pOldFinesse = dynamic_cast<CFinesse*> (pUsedPlay);
			...
			if ((nTrickWinner == GetPlayerPosition()) || (nTrickWinner == GetPartnerPosition()))
			{
				...
				{
					status << "!5PLTST1! The [" & pNewFinesse->GetName() & "] targets " & app_->PositionToString(nNewTarget) &
							  ", while we successfully finessed against " & app_->PositionToString(nOldTarget) &
...
//		ASSERT(pFinesse);
		...
		if (pFinesse->GetTargetPos() == CPlay::IN_DUMMY)
			nOpponent = app_->GetNextPlayer(m_pPlayer->GetPosition());	// LHO
		else	// finessing in hand
			nOpponent = app_->GetPrevPlayer(m_pPlayer->GetPosition());	// RHO
        ...
        app_->PositionToString(nOpponent) & " has shown out of the suit, so delete the play.\n";
			AdjustPlayCountForDeletedPlay(m_playPlan[nIndex]);
			m_playPlan.RemovePlay(nIndex, FALSE);
			m_discardedPlays.AppendPlay(pPlay);				
			nIndex--;
	...
	if (!bSequenced && (numSolidPlays >= m_numTricksLeftToBeMade)) // ?????
	{
		...
		if (numUsableRuffs > 0)
		{
			...
			else
			{
				...
				strPlan += app_->FormString("ruffing %d card(s)", numUsableRuffs);
			}
			delete pRuffs;
		}
...
//				if ( ((pPlay->GetPlayType() == CPlay::CASH) || (pPlay->GetPlayType() == CPlay::TRUMP_PULL)) && 
	...
	if (app_->GetDeal()->GetCurrentPlayerPosition() == GetPlayerPosition())
...
//	for(int i=0;i<numPlays;i++)
	...
	for(int i=numPlays-1;(i>0) && (numIters>0);i--,numIters--)	// go in reverse to preserve the order
	{
		...
		{
			int nCurrentHand = (app_->GetDeal()->GetCurrentPlayer() == m_pPlayer) ? CPlay::IN_HAND : CPlay::IN_DUMMY;
...
int CDeclarerPlayEngine::FindHoldUpPlays(CPlayList& playList, BOOL bExcludeTrumpSuit)
{
	...
	if (ISSUIT(app_->GetDeal()->GetTrumpSuit()) || (nRound >= 2))
		return 0;
	...
	CCard* pLeadCard = app_->GetDeal()->GetCurrentTrickCardByOrder(0);
	int nSuitLed = pLeadCard->GetSuit();
...
//		numSuitTopCards = suit.GetNumTopCards();
	...
	
	{
		...
		if (suit.GetNumMissingSequences() > 0)
		{
			...
			int numTricksLeft = 13 - app_->GetDeal()->GetNumTricksPlayed();
			//
...
//					pRequiredPlayList->Add(missingTopCards[0]);
			...
			if ((app_->GetDeal()->GetCurrentPlayer() == m_pPlayer) &&
...
//	if ((dummy.GetLength() == 0) || (declarer.GetLength() == 0))
	...
	if (bHaveLosers && (suit.GetNumMissingInternalHonors() > 0))
	{
						...
						(declarer.GetNumBodyCards() > 0))
		{
			...
			int nLHO = app_->GetNextPlayer(m_pPlayer->GetPosition());
	...
	if ((!bDummyHasCommandingCard) && bHaveLosers && (dummy.GetNumHonors() >= 1))
	{
        ...
        app_->PositionToString(app_->GetNextPlayer(m_pPartner->GetPosition())) & ".\n";
			// advance the count
			numFinesses++;
		}
	}
	...
	if ((!bDeclarerHasCommandingCard) && bHaveLosers && (declarer.GetNumHonors() >= 1))
	{
        ...
        app_->PositionToString(app_->GetNextPlayer(m_nPosition)) & ".\n";
			// advance the count
			numFinesses++;
		}
	}
	...
	CCard* pCardLed = app_->GetDeal()->GetCurrentTrickCardByOrder(0);
	if ((app_->GetDeal()->GetNumCardsPlayedInRound() == 1) && (app_->GetDeal()->GetCurrentPlayer() == m_pPartner) &&
	...
	// or in hand
	if ((app_->GetDeal()->GetNumCardsPlayedInRound() == 1) && (app_->GetDeal()->GetCurrentPlayer() == m_pPlayer) &&
