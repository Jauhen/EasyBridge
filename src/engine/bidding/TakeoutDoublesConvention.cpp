...
BOOL CTakeoutDoublesConvention::TryConvention(const CPlayer& player,
  ...
  if (((bidState.nLHOBid > BID_PASS) || (bidState.nRHOBid > BID_PASS)) &&
    ...
    (bidState.fCardPts >= app_->GetSettings()->OpenPoints(12)) &&
  ...
  if ((nOppBidLevel == 3) && (bidState.fCardPts < app_->GetSettings()->OpenPoints(13)))
    return FALSE;
  ...
  if ((bidState.m_numBidTurns == 1) && (bidState.m_nBid == BID_DOUBLE) &&
    ...
    // need 14+ HPCs
    if (bidState.fCardPts < app_->GetSettings()->OpenPoints(14)) {
  ...
  int n2NTMin = app_->GetCurrentConventionSet()->GetNTRangeMin(2);
  int n2NTMax = app_->GetCurrentConventionSet()->GetNTRangeMax(2);
  int n3NTMin = app_->GetCurrentConventionSet()->GetNTRangeMin(3);
  int n3NTMax = app_->GetCurrentConventionSet()->GetNTRangeMax(3);
  double fCardPts = bidState.fCardPts;
  if ((bidState.bBalanced) &&
    (((fCardPts >= app_->GetSettings()->OpenPoints(n1NTMin)) && (fCardPts <= n1NTMax) && (nOppBidLevel == 1)) ||
    ((fCardPts >= app_->GetSettings()->OpenPoints(n2NTMin)) && (fCardPts <= n2NTMax) && (nOppBidLevel <= 2)) ||
      ((fCardPts >= app_->GetSettings()->OpenPoints(n3NTMin)) && (fCardPts <= n3NTMax) && (nOppBidLevel <= 3))))
    return FALSE;
...
BOOL CTakeoutDoublesConvention::RespondToConvention(const CPlayer& player,
  ...
  if (nConventionStatus == CONV_INACTIVE) {
    ...
    bidState.m_fPartnersMin = app_->GetSettings()->OpenPoints(nPartnersPoints);
    bidState.m_fPartnersMax = Min(app_->GetSettings()->OpenPoints(22), 40 - bidState.fCardPts);
    ...
    // see if RHO has bid something
      ...
      if (((nLevel == 1) && (bidState.fCardPts >= PTS_NT_GAME - 7 - nPartnersContrib)) ||
        ...
        status << "TKOTR14! However, assuming partner has 12+ HCPs, that amount combined with our " &
          ...
          " pts, which is enough for us to bid " & BidToFullString(nBid) & ".\n";
        bidState.SetBid(nBid);
...
//				bidState.SetConventionStatus(this, CONV_FINISHED);
    ...
    //
    if ((bidState.bBalanced) && (fCardPts >= app_->GetSettings()->OpenPoints(6)) && (fCardPts < app_->GetSettings()->OpenPoints(10))) {
      nBid = BID_1NT;
    ...
    //
    if (fPts < app_->GetSettings()->OpenPoints(10)) {
      ...
      if (nSuit == nEnemySuit) {
        ...
        if (hand.GetSuitLength(nAltSuit) >= 4) {
          status << "TKOTR20a! Our longest suit of " & CCard::SuitToString(nSuit) &
            " happens to be the enemy suit, so respond in the next-longest suit of " &
            CCard::SuitToString(nAltSuit) & ".\n";
          nSuit = nAltSuit;
        } else {
          // can't bid at all, with only 9 pts
          nBid = BID_PASS;
          status << "TKOTR20c! Our longest suit of " & CCard::SuitToString(nSuit) &
        ...
        if ((nSuit != nOrigSuit) &&
          ...
          status << "TKOTR21! With " & fCardPts & "/" & fPts & " pts, respond in the major suit of " &
            CCard::SuitToString(nSuit) & " (even though it's not our longest suit) at a bid of " & BidToFullString(nBid) & ".\n";
        } else {
          status << "TKOTR22! With " & fCardPts & "/" & fPts & " pts, respond with our longest suit of " &
            CCard::SuitToString(nSuit) & " at a bid of " & BidToFullString(nBid) & ".\n";
        }
      }
      // done
      bidState.SetBid(nBid);
    ...
    if (nSuit != NONE) {
      ...
      if (fPts >= app_->GetSettings()->OpenPoints(13)) {
        nBid = bidState.GetGameBid(nSuit);
        status << "TKOTR20! With " & fCardPts & "/" & fPts &
          " pts and a " & nLength & "-card " & CCard::SuitToSingularString(nSuit) &
          " suit, jump to game at " & BidToFullString(nBid)  & ".\n";
      } else {
        nBid = MAKEBID(nSuit, 3);
        status << "TKOTR22! With " & fCardPts & "/" & fPts &
          " pts and a " & nLength & "-card " & CCard::SuitToSingularString(nSuit) & " suit, " &
    ...
    if (bidState.bBalanced) {
      ...
      if (fCardPts >= app_->GetSettings()->OpenPoints(13)) {
        if (hand.IsSuitStopped(nEnemySuit) && (nEnemySuit != NOTRUMP)) {
          nBid = BID_3NT;
          status << "TKOTR30! With " & fCardPts &
            " HCPs, a balanced hand, and the opponents' " & CCard::SuitToSingularString(nEnemySuit) &
            " stopped, respond with a bid of " & BidToFullString(nBid) & ".\n";
        } else if (nEnemySuit == NOTRUMP) {
          nBid = BID_3NT;
          status << "TKOTR30! With " & fCardPts &
            " HCPs, a balanced hand, respond with a bid of " & BidToFullString(nBid) & ".\n";
        } else {
          nBid = BID_2NT;
          status << "TKOTR32! We have " & fCardPts &
            " HCPs and a balanced hand, which would be enough for a response of 3NT, but the opponents' "
            & CCard::SuitToSingularString(nEnemySuit) & " is not securely stopped, so just bid " & BidToFullString(nBid) & ".\n";
        }
      } else {
    ...
    //
    if ((fPts >= app_->GetSettings()->OpenPoints(13)) && (nEnemySuit != NOTRUMP)) {
      ...
      status << "TKOTR55! With " & fCardPts & "/" & fPts &
        " pts and no long major suit, jump to " & BidToFullString(nBid) &
    ...
    if (fMinTPPoints < app_->GetSettings()->OpenPoints(20)) {
    ...
    if (nPartnersSuit == nPreviousSuit) {
      ...
      if (fMinTPPoints < PTS_GAME) {
        ...
        if ((bidState.nSuitStrength[nSuit] >= SS_MARGINAL_OPENER) &&
          ...
          status << "TKOTR68! With " & fMinTPPoints & "-" & fMaxTPPoints &
            ...
            ", we shift to " & CCard::SuitToString(nSuit) & " at a bid of " & BidToFullString(nBid) & ".\n";
          bidState.SetBid(nBid);
          bidState.SetConventionStatus(this, CONV_RESPONDED_ROUND2);
        } else {
      ...
      if (bidState.RebidSuit(SUIT_MAJOR, REBID_AT_4, PTS_GAME, PTS_SLAM - 1, LENGTH_6, SS_SELFSUPPORTING, HONORS_3)) {
        status << "TKOTR80! With a self-supporting " & bidState.szPrefS &
          ...
          "+ team points, jump to game at " & BidToFullString(bidState.m_nBid)  & ".\n";
        bidState.SetConventionStatus(this, CONV_RESPONDED_ROUND2);
        return TRUE;
      }
      ...
      // else rebid our suit at the cheapest level
        ...
        status << "TKOTR85! With " & fMinTPPoints & "-" & fMaxTPPoints &
          ...
          " support for partner's " & bidState.szPS &
          ", we rebid our " & bidState.szPVSS & " suit at " & BidToFullString(nBid) & ".\n";
        bidState.SetBid(nBid);
        bidState.SetConventionStatus(this, CONV_RESPONDED_ROUND2);
        return TRUE;
      }
  ...
  if (bidState.GetConventionStatus(this) == CONV_INVOKED_ROUND1) {
    ...
    if (nPartnersSuit == NOTRUMP) {
      ...
      if (nPartnersBid == BID_1NT) {
        ...
        // partner has 13+ HCPs???
        status << "TKRb21! Partner's response of " & BidToFullString(nPartnersBid) &
      ...
      status << "TKRb29! Partner's response of " & BidToFullString(nPartnersBid) &
      ...
      if (nPartnersSuit == nEnemySuit) {
        ...
        status << "TKORb41! Partner has jumped to game in " & CCard::SuitToString(nPartnersSuit) &
        ...
        status << "TKORb42! Partner has made a jump response of " & BidToFullString(nPartnersBid) &
        ...
        // partner had <= 9 pts
        status << "TKORb43! Partner has made a minimum response of " & BidToFullString(nPartnersBid) &
