...
std::string Deal::WriteFilePBN() {
  ...
  fe.WriteComment((const char*)app_->FormString("File generated by Easy Bridge version %s", app_->GetProgramVersionString()));
  fe.WriteComment("");
  ...
  // Event tag
  fe.WriteLine(TAG_EVENT, (const char*)app_->FormString("%s Game", app_->GetProgramTitle()));
  ...
  fe.WriteLine(TAG_DATE, (const char*)time.Format("%Y.%m.%d"));
  ...
  */
  ...
  // Dealer Tag
  fe.WriteLine(TAG_DEALER, (const char*)app_->FormString("%c", app_->PositionToChar(m_nDealer)));
  ...
  if (ISPOSITION(m_nDeclarer))
    fe.WriteLine(TAG_DECLARER, (const char*)app_->FormString("%c", app_->PositionToChar(m_nDeclarer)));
  else
    fe.WriteLine(TAG_DECLARER, "?");

  // Contract Tag
  if (ISBID(m_nContract))
    fe.WriteLine(TAG_CONTRACT, (const char*)app_->ContractToString(m_nContract, 0));
  else
    fe.WriteLine(TAG_CONTRACT, "?");

  // Result tag
  if (m_numTricksPlayed == 13)
    fe.WriteLine(TAG_RESULT, (const char*)app_->FormString("\"%d\"", m_numTricksWon[m_nContractTeam]));
  else
  ...
  // write out auction
  //
  std::string strBids = (const char*)app_->FormString("[AUCTION \"%c\"]", app_->PositionToChar(m_nDealer));
  if (m_numBidsMade > 0)
    strBids += "\n";
  nPos = m_nDealer;
  for (int i = 0; i<m_numBidsMade; i++) {
    strBids += (const char*)app_->FormString("%s ", app_->BidToPBNString(m_nBiddingHistory[i]));
    nPos = app_->GetNextPlayer(nPos);
  ...
  // write out plays
  //
  std::string strPlays = (const char*)app_->FormString("[PLAY \"%c\"]", app_->PositionToChar(m_nGameLead));
  ...
  // Generator Tag
  fe.WriteLine(TAG_GENERATOR, (const char*)app_->FormString("Easy Bridge version %s", app_->GetProgramVersionString()));
