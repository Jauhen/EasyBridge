...
void CEasyBView::SetCurrentMode(int nMode) {
  ...
  if (nMode == MODE_NONE && state_->IsAllCardVisible())
...
void CEasyBView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) {
...
void CEasyBView::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if ((m_nCurrMode != MODE_WAITCARDPLAY) && !state_->IsAllCardVisible())
    return;
  ...
  if (!bFound) {
    ...
    } else if (state_->IsAllCardVisible()) {
...
void CEasyBView::OnUpdateDistributeRandom(CCmdUI* pCmdUI) {
  if (state_->IsAllCardVisible() && m_numCardsUnassigned > 0)
    pCmdUI->Enable(TRUE);
  else
    pCmdUI->Enable(FALSE);
}
...
void CEasyBView::OnUpdateBidCurrentHand(CCmdUI* pCmdUI) {
  ...
  //
  if ((!pDOC->GetDeal()->IsHandsDealt()) || state_->IsAllCardVisible())
    pCmdUI->Enable(FALSE);
  else
    pCmdUI->Enable(TRUE);
  /*
