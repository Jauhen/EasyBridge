 ...
 * $Date: 8/31/99 1:11a $
 * $Revision: 10 $
 * $Archive: /CodeJockey/CJLibrary/CJDockContext.cpp $
 *
...
DWORD CCJDockContext::CanDockDockBar()
{
	...
	dwDock = CanDockDockBar(m_rectFrameDragHorz, m_dwDockStyle);
	return dwDock;
}
...
DWORD CCJDockContext::CanDockDockBar(CRect rect, DWORD dwDockStyle, CDockBar** ppDockBar)
{
	...
	while (pos != NULL)
	{
		...
		if( !pDockBar->IsKindOf(RUNTIME_CLASS(CCJSizeDockBar)) )
			continue;
		if (pDockBar->m_bFloating || !pDockBar->IsWindowVisible())
			continue;
		CRect rectBar, rect2;
		pDockBar->GetWindowRect(&rectBar);
		rect2 = rectBar;
		if( pDockBar->m_dwStyle & CBRS_ORIENT_HORZ )
			rect.bottom = rect.top + (rectBar.Height()!=0?rectBar.Height():m_rectDragHorz.Height());
		else
			rect.right = rect.left + (rectBar.Width()!=0?rectBar.Width():m_rectDragVert.Width());
		...
		if (rect2.IntersectRect(rect2, rect)) {
			if( pDockBar->m_dwStyle & CBRS_ORIENT_HORZ )
			{
				if (rectBar.Height() != 0) {
					if ((m_dwStyle & CBRS_ALIGN_ANY) == (pDockBar->m_dwStyle & CBRS_ALIGN_ANY)
						 && !m_pBar->m_pDockBar->m_bFloating) {
						if (pDockBar->GetDockedVisibleCount() == 1)
							m_rectDragHorz.SetRect(rectBar.left, m_rectDragHorz.top, 
								rectBar.right, m_rectDragHorz.top + rectBar.Height());
						else
							m_rectDragHorz.SetRect(rect.left, m_rectDragHorz.top, 
								rect.left+((CCJControlBar*)m_pBar)->m_sizeHorz.cx, m_rectDragHorz.top + rectBar.Height());
					}
					else
						m_rectDragHorz.SetRect(rect.left, m_rectDragHorz.top, 
							rect.right, m_rectDragHorz.top + rectBar.Height());
				}
				else
					m_rectDragHorz.SetRect(rectBar.left, m_rectDragHorz.top, 
						rectBar.right, m_rectDragHorz.top + m_rectDragHorz.Height());
			}
			else
			{
				if (rectBar.Width() != 0) {
					if ((m_dwStyle & CBRS_ALIGN_ANY) == (pDockBar->m_dwStyle & CBRS_ALIGN_ANY)
						 && !m_pBar->m_pDockBar->m_bFloating) {
						if (pDockBar->GetDockedVisibleCount() == 1)
							m_rectDragVert.SetRect( m_rectDragVert.left, rectBar.top,
								m_rectDragVert.left + rectBar.Width(), rectBar.bottom);
						else
							m_rectDragVert.SetRect( m_rectDragVert.left, rect.top,
								m_rectDragVert.left + rectBar.Width(), rect.top+((CCJControlBar*)m_pBar)->m_sizeVert.cy);
					}
					else
						m_rectDragVert.SetRect( m_rectDragVert.left, rect.top,
							m_rectDragVert.left + rectBar.Width(), rect.bottom);
				}
				else
					m_rectDragVert.SetRect( m_rectDragVert.left, rectBar.top,
						m_rectDragVert.left + m_rectDragVert.Width(), rectBar.bottom);
			}
			if (ppDockBar != NULL)
				*ppDockBar = pDockBar;
...
void CCJDockContext::EndDragDockBar()
{
	...
	if (m_dwOverDockStyle != 0)
	{
		...
		BOOL bAdd = !( (m_dwStyle & CBRS_ALIGN_ANY) == (pDockBar->m_dwStyle & CBRS_ALIGN_ANY)
			 && !m_pBar->m_pDockBar->m_bFloating);
		...
		pDockBar->CalcSizeBarLayout((CCJControlBar*)m_pBar, bAdd);
...
//        m_pDockSite->RecalcLayout( FALSE );
	...
	{
		CCJSizeDockBar* pDockBar = (CCJSizeDockBar*)m_pBar->m_pDockBar;
		...
		if (pDockBar != m_pBar->m_pDockBar)
		{
			pDockBar->CalcSizeBarLayout((CCJControlBar*)m_pBar, -1);
			((CCJSizeDockBar*)m_pBar->m_pDockBar)->CalcSizeBarLayout((CCJControlBar*)m_pBar, 1);
			CMiniDockFrameWnd* pDockFrame =
				(CMiniDockFrameWnd*)m_pBar->m_pDockBar->GetParent();
...
CCJSizeDockBar* CCJDockContext::GetDockBar(DWORD dwOverDockStyle)
{
	...
	{
		ASSERT_KINDOF(CFrameWnd, m_pDockSite);
		dw = CanDockDockBar(m_rectFrameDragHorz,
...
//				CRect rect = (m_dwDockStyle & CBRS_ORIENT_VERT)?m_rectDragVert : m_rectDragHorz;
