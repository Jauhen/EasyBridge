...
/****************************************************************************
 *
 * $Date: 10/25/99 10:54p $
 * $Revision: 9 $
 * $Archive: /CodeJock/CJLibrary/GfxOutBarCtrl.cpp $
 *
 * $History: GfxOutBarCtrl.cpp $
 * 
 * *****************  Version 9  *****************
 * User: Kirk Stowell Date: 10/25/99   Time: 10:54p
 * Updated in $/CodeJock/CJLibrary
 * Added resource includes for static builds, type cast popup menu calls
 * to call CMenu base class.
 * 
 * *****************  Version 8  *****************
...
CGfxOutBarCtrl::CGfxOutBarCtrl()
{
	...
	m_strRemoveItem.LoadString(IDS_REMOVEITEM);
	m_strRenameItem.LoadString(IDS_RENAMEITEM);
}
...
void CGfxOutBarCtrl::SetFolderText( const int index, LPCTSTR text )
{
...
void CGfxOutBarCtrl::SetItemText( const int index, LPCTSTR text )
{
...
int CGfxOutBarCtrl::AddFolder( LPCTSTR cFolderName, const DWORD exData )
{
...
CGfxOutBarCtrl::CBarFolder::CBarFolder( LPCTSTR name, DWORD exData)
{
	cName  = NULL;
	dwData = exData;
...
int CGfxOutBarCtrl::InsertItem(const int folder, const int index, LPCTSTR text, const int image, const DWORD exData)
{
...
int CGfxOutBarCtrl::CBarFolder::InsertItem(int index, LPCTSTR text, const int image, const DWORD exData)
{
...
CGfxOutBarCtrl::CBarItem::CBarItem( LPCTSTR name, const int image, DWORD exData )
{
	cItem		= NULL;
	iImageIndex = image;
	dwData		= exData;
...
void CGfxOutBarCtrl::HighlightItem(const int index, const bool bPressed)
{
	...
	dc.RestoreDC(isdc);
	GetOwner()->SendMessage(WM_OUTBAR_NOTIFY,NM_OB_ITEMHOVER,index);
}
...
void CGfxOutBarCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (iHitInternal1 == htItem)
	{
		if (dwFlags&fRemoveItems || dwFlags&fEditItems)
		{
			...
			if (dwFlags&fRemoveItems)
				((CMenu*)pPopup)->AppendMenu(MF_STRING, ID_GFX_REMOVEITEM, m_strRemoveItem);

			if (dwFlags&fEditItems)
				((CMenu*)pPopup)->AppendMenu(MF_STRING, ID_GFX_RENAMEITEM, m_strRenameItem);
		}
	}
	else if (iHitInternal1 == htFolder)
	{
		...
		{
			((CMenu*)pPopup)->AppendMenu(MF_SEPARATOR);
...
int CGfxOutBarCtrl::AddFolderBar( LPCTSTR pFolder, CWnd* pSon, const DWORD exData )
{
