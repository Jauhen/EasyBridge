...
BOOL CCJControlBar::Create(CWnd *pParentWnd, UINT nID, LPCTSTR lpszWindowName, CSize sizeDefault, DWORD dwStyle, BOOL bFlatBorders)
{
	...
	if (!m_bFloating)
	{
		if (m_nBarID == AFX_IDW_SIZEBAR_LEFT || m_nBarID == AFX_IDW_SIZEBAR_RIGHT)
		{
			rect.SetRect(0, 0, m_sizeVert.cx, m_sizeVert.cy);
			m_sizeDesired = m_sizeVert;
		}
		else if (m_nBarID == AFX_IDW_SIZEBAR_TOP || m_nBarID == AFX_IDW_SIZEBAR_BOTTOM)
		{
			rect.SetRect(0, 0, m_sizeHorz.cx, m_sizeHorz.cy);
			m_sizeDesired = m_sizeHorz;
		}
	}
...
void CCJControlBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
	...
	else if( !m_bDragging && !(sizeTemp.cx == 0 && sizeTemp.cy == 0))
	{
        if( IsHorzDocked() ) {
            m_sizeHorz.cx = sizeTemp.cx;
        } else {
            m_sizeVert.cy = sizeTemp.cy;
        }
    }
...
void CCJControlBar::OnButtonMinimize()
{
	if (!m_bMaximized) {
		...
		((CCJSizeDockBar *)GetParent())->Average(this);
		m_bMaximized = FALSE;
	}
}
...
//			m_strSection += _T("\\CodeJock");
			pApp->WriteProfileInt( m_strSection, _T("CJMRUFloatCX"), m_sizeFloat.cx );
			pApp->WriteProfileInt( m_strSection, _T("CJMRUFloatCY"), m_sizeFloat.cy );
			pApp->WriteProfileInt( m_strSection, _T("CJMRUVertCX"), m_sizeVert.cx );
			pApp->WriteProfileInt( m_strSection, _T("CJMRUVertCY"), m_sizeVert.cy );
			pApp->WriteProfileInt( m_strSection, _T("CJMRUHorzCX"), m_sizeHorz.cx );
			pApp->WriteProfileInt( m_strSection, _T("CJMRUHorzCY"), m_sizeHorz.cy );
			pApp->WriteProfileInt( m_strSection, _T("CJMRUFloating" ), IsFloating() );
			return;
		}
...
void CCJControlBar::LoadBarState(CSize& size, int nID)
{
	...
	while( TRUE )
	{
		...
		if( nBarID == m_nID ) {
			m_nBarID	= pApp->GetProfileInt(m_strSection, _T("MRUDockID"), 0);
//			m_strSection += _T("\\CodeJock");
			size.cx     = pApp->GetProfileInt( m_strSection, _T("CJMRUFloatCX"), size.cx );
			size.cy     = pApp->GetProfileInt( m_strSection, _T("CJMRUFloatCY"), size.cy );
			m_sizeFloat = size;
			m_sizeVert.cx = pApp->GetProfileInt( m_strSection, _T("CJMRUVertCX"), size.cx );
			m_sizeVert.cy = pApp->GetProfileInt( m_strSection, _T("CJMRUVertCY"), size.cy );
			m_sizeHorz.cx = pApp->GetProfileInt( m_strSection, _T("CJMRUHorzCX"), size.cx );
			m_sizeHorz.cy = pApp->GetProfileInt( m_strSection, _T("CJMRUHorzCY"), size.cy );
			m_bFloating = pApp->GetProfileInt( m_strSection, _T("CJMRUFloating" ), m_bFloating );
			return;
		}
